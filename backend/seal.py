...
r_learn = 60 #参数需要学习得知最小值为多少比较合适，下同
e_learn = 0.8
point_learn = 5000
wh_learn = 2.2
w_learn = 100
...
def convet_pdf(pdf_name):
    ...
    tem = pdf_name.split(".")
    file_name = tem[0]
    doc = fitz.open(pdf_root+pdf_name)
    i = 0
    img_name_list = []
    for page in doc:
        ...
        pix.save(img_root+img_name)#将图片写入指定的文件夹内
...
def get_is_seal(json_name):
    '''
    参数：json_name：存储画印章json文件的文件名（根目录已给出）
    输出：bool
    函数功能：判断画面是否有印章
    '''
    with open(json_root+json_name, "r") as f:
...
def draw_seal(json_name,img_name):
    '''
    ...
    cv2.imwrite(img_root+img_out, img)



def check_seal(img_name):
    '''
    ...
    for mask in mask_list:
        ...
        for cnt in cnts:
            if cv2.contourArea(cnt) > point_learn:
                ...
                if shape_type == "圆":
                    ...
                    r = min(a,b) #圆章
                    rm = max(a,b)
                    if r>=r_learn and e <= e_learn and rm <min(final_width,final_height)/3:
                        print(e)
                        info = {
                            'type':'elipse',
                            'center':(int(x),int(y)),
                        ...
                        (x,y),(w,h),ang =rect
                        print(rect)
                        if max(w,h)/min(w,h) <wh_learn and max(w,h) >w_learn and max(w,h) <max(final_width,final_height)/3:
                            info = {
                                'type':"rectangle",
                                'center':(int(x),int(y)),
                                'shape':(int(w),int(h)),
                    ...
                    (x,y),(w,h),ang =rect
                    print(rect)
                    if max(w,h)/min(w,h) and max(w,h) >w_learn and max(w,h) <max(final_width,final_height)/3:
                        info = {
                            'type':"rectangle",
                            'center':(int(x),int(y)),
                            'shape':(int(w),int(h)),
    ...
    with open(json_root+img_name+'.json', 'w') as json_file:
        json_file.write(json_str)

    draw_seal(img_name+'.json',img_name)
